...
ship_embedding = Embedding(output_dim=200, input_dim=40000, input_length=100)(ship_input)
lstm_ships = LSTM(32)(ship_embedding)
auxiliary_output = Dense(len(ship_ID_properties["EquipmentIDs"]), activation='sigmoid', name='aux_output')(lstm_ships)
...
main_output = Dense(len(ship_ID_properties["EquipmentIDs"]), activation='softmax', name='main_output')(x)
...
model.fit({'ship_input': training_set[0], 'modules_input' : training_set[1]},
